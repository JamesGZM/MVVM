...
public class RxPermissions {
    ...
    public <T> ObservableTransformer<T, Boolean> ensure(final String message, final String... permissions) {
        return new ObservableTransformer<T, Boolean>() {
            @Override
            public ObservableSource<Boolean> apply(Observable<T> o) {
                return request(message, o, permissions)
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public <T> ObservableTransformer<T, Permission> ensureEach(final String message, final String... permissions) {
     ...
     * to ask the user if he allows the permissions.
     */
    public <T> ObservableTransformer<T, Permission> ensureEachCombined(final String message, final String... permissions) {
        return new ObservableTransformer<T, Permission>() {
            @Override
            public ObservableSource<Permission> apply(Observable<T> o) {
                return request(message, o, permissions)
    ...
    public Observable<Boolean> request(final String message, final String... permissions) {
        return Observable.just(TRIGGER).compose(ensure(message, permissions));
    }
     ...
     */
    @SuppressWarnings({"WeakerAccess", "unused"})
    public Observable<Permission> requestEach(final String message, final String... permissions) {
        return Observable.just(TRIGGER).compose(ensureEach(message, permissions));
    }
     ...
     * of your application</b>.
     */
    public Observable<Permission> requestEachCombined(final String message, final String... permissions) {
        return Observable.just(TRIGGER).compose(ensureEachCombined(message, permissions));
    }

    private Observable<Permission> request(final String message, final Observable<?> trigger, final String... permissions) {
        ...
        return oneOf(trigger, pending(permissions))
                .flatMap(new Function<Object, Observable<Permission>>() {
                    ...
                    public Observable<Permission> apply(Object o) {
                        return requestImplementation(message, permissions);
                    }
                });
    }
    ...
    private Observable<Permission> requestImplementation(final String message, final String... permissions) {
        ...
        if (!unrequestedPermissions.isEmpty()) {
            ...
            requestPermissionsFromFragment(message, unrequestedPermissionsArray);
        }
    ...
    @TargetApi(Build.VERSION_CODES.M)
    void requestPermissionsFromFragment(String message, String[] permissions) {
        mRxPermissionsFragment.get().log("requestPermissionsFromFragment " + TextUtils.join(", ", permissions));
        mRxPermissionsFragment.get().requestPermissions(message, permissions);
    }
